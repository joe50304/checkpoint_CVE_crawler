...
2. 一張一張看紙條，只撿起「**超危險**或**很危險**」而且是「**你指定的天數內**貼上去」的紙條（預設 7 天，最多可以往回找 90 天）。
...
# 往回找久一點，最多 90 天
python crawler.py --days 30
python crawler.py --days 90
```
...
| 參數 | 用途 | 預設值 |
|---|---|---|
| `--days` | 要抓幾天內的公告，可以填 `1` 到 `90` 的整數 | `7` |
...
**Q：`--days` 可以填多少？**
A：`1` 到 `90` 的整數。超出範圍會直接被擋下來，畫面顯示 `argument --days: days must be between 1 and 90`。
天數拉大時，小機器人會自動多翻幾頁，花的時間也會比較久。
想放寬上限的話，把 `crawler.py` 裡 `days_arg()` 的 `1 <= n <= 90` 改掉即可。
